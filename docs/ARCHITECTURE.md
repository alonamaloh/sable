...
There is **no external SMT solver**. Routine obligations are discharged by an automation portfolio inside Lean (`sable_auto`: `assumption`/`omega`/`sable_instantiate` quantifier instantiation (ADR 0082, plus `#[fact]` ghost theorems per ADR 0084)/`simp_all`/`sable_cases` match splitting (ADR 0081)/budgeted `grind`, see `lean/Sable/Auto.lean`). Lean elaborates every emitted declaration, and the kernel checks the resulting declaration and proof term relative to its environment. Priority zero must additionally authenticate that environment's complete axiom dependency closure. The grind tier runs under a heartbeat budget (`sable.grindHeartbeats`, ADR 0011): exceeding it fails the obligation promptly instead of churning, and a success spending ≥ 1/5 of the budget is reported as a warning diagnostic — obligation name, clause span, and a minimized `discharge` suggestion from `grind?` — with the corpus held clean for the warnings it currently recognizes. Stage-1 translation trust (design §10.1) is the Rust checker, VC generator, and emitter. The consolidated ownership/control plans narrow duplicated decisions inside that boundary; they do not retire it.
...
check                                 capture one immutable ProofEnvironment before
  ...
  │                                   Lean runtime task manager disabled so one serialized
  │                                   Lake build workload owns at most one Lean runtime;
  ...
  │                                   set plus the parser auditor and observational
  │                                   declaration-inventory executable by SHA-256 are last;
...
- **The first Boolean aggregate slice is deliberately local and
  ...
  type has no LLVM representation in any position — and a stored RECORD
  option field keeps `interp.option_position_unsupported`. Init and method
  parameters admit the same copyable option and `bool` shapes plain calls do
  (`check::member_param_ty`), and class fields store them
  (`check::class_field_ty`), with the stored state carrying the integer
  payload's range fact over `.value`. Not admitted
  anywhere: option parameters with a type-parameter payload
  (`type.option_param`), trait-method option parameters
  (`type.trait_param_unsupported`), option-valued record fields,
  trait or impl
...
```
docs/design/       normative language design + original research goals
docs/PLAN.md       provisional priorities, gates, and deferred scope
...
corpus/verifies/   programs intended to reach the strongest verification status
                   (currently provisional under PLAN priority zero)
