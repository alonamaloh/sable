...
Collapse the `///` lines and the function reads as plain C with a two-line contract. *(v0.4+: locals are immutable unless declared `mut` — ADR 0016; the older examples below predate the marker and should be read with it implied on mutated locals.)* The third invariant is representative of verified programming in practice: the compiler forces you to state *why* nothing overflows, and the bound lives with the proof, not the interface. Invariants change constantly during proof development; because those edits are `///`-only, the executable program has zero diff noise.
...
> **Implementation deviation (ADR 0072).** Rules 2 and 3 speak of a borrow *living* and the owner being frozen or inaccessible *meanwhile*, which presumes a borrow-liveness discipline. The compiler has none: there is no loan map, and `Place::contains`/`overlaps` relates borrows to owners only within a single call's argument list. So a borrow is admitted only where that one rule is enough to enforce exclusivity — **written at a call, bound to a parameter for the length of that call**. It is not a local binding: `var view = &mut a;` is refused by `type.borrow_local_unsupported`, for every referent, because a second name for one storage is precisely what nothing in the tree tracks. `swap(&mut x, &mut x)` is a type error exactly as the example above says; `var v = &mut x; swap(&mut x, &mut v);` is refused at the binding instead. Rules 2 and 3 as written are what a real loan-liveness discipline would license, and they stay the target.
...
- **Semantics**: small-step for statements, big-step for the pure expression layer (calls are A-normalized to statement level, so expressions cannot diverge); deterministic given the allocation-capacity parameter `cap` (OOM is the defined trap above it; soundness quantifies over `cap`); left-to-right, short-circuiting. Formalized in Lean as inductive relations (`lean/Sable/SVM.lean`, core subset — classes, calls, and ghost state still scoped out). This artifact is the language's meaning — there is no prose abstract machine to disagree with it. Determinism, totality, and progress are kernel-checked theorems, corollaries of a functional evaluator proven to agree with the rules in both directions (`lean/Sable/SVMEval.lean`, ADR 0017).
...
Concurrency (rely-guarantee with ghost resources), the `unsafe` sublanguage and FFI (design deliverable of the allocator benchmark, and the gate to any adoption claim — §5), closures capturing borrows, floating-point verification beyond range facts, surface lifetime annotations for non-lexical borrows, richer generics (Boolean/aggregate and nested generic-class arguments, multiple/inherited trait bounds — integer arguments and direct ordinary class owners exist per ADR 0006, law-carrying bounds per ADR 0007, and integer-only type-precondition proof reuse per ADR 0009), and full first-class class values (shared borrows/returns/field reads exist per ADR 0010; moves, `&mut` borrows, and class-valued fields landed with the place engine, destruction semantics, and unified transfer, ADRs 0023/0029/0030). Each has known literature; none blocks the core.

One generics decision is committed upfront rather than left to the benchmark, because it constrains everything downstream: **monomorphization before VC generation** (Verus's route). *(v0.4+ implementation: generics per ADR 0006 use type parameters on classes and functions, always-explicit instantiation (`Vec<i32>::with_capacity(4)`), and expansion before ordinary checking so executable/backend stages see no type variable. The concrete argument domain is the eight integer types plus a direct ordinary, non-generic class; Boolean, aggregate, and nested generic-class arguments remain closed. `T` is substituted even inside proof-clause text. All-integer instances retain legacy names and ADR 0009's `Sable.IntModel` proof reuse; any instance containing a class owner uses an injective structural name, receives `ProofReuse::None`, and is independently checked and verified with its affine `Ty::Class`. Law-carrying trait bounds landed with the hash-map benchmark (ADR 0007): a trait pairs a spec-level function (`/// spec hash : int → int`, referenced `Self::hash`/`K::hash`) with a program method contracted against it — the equation is the law that restores determinism and spec-level application; impls provide the spec function as a ghost def plus bodies verified against the trait contract, and monomorphization consumes the whole mechanism. Current bound limits remain integer implementors, one bound per parameter, no inheritance or default bodies.)* Generic code is specialized before ordinary verification conditions are produced, while retained ADR 0009 templates are checked separately over their abstract integer model. The hash-map benchmark drives the surface design — trait syntax, law-carrying bounds, contract inheritance — not the compilation strategy. Retrofitting generics into a VCgen and a soundness proof is famously painful; fixing this early is deliberate risk reduction, at the known cost of code-size blowup and no polymorphic compilation, both acceptable for a verification-first systems language.
