...
Original north star for v0.1 — **verify `binary_search` and insertion sort end-to-end, with no hand-waving** — was reached 2026-08-08/09; the corpus now also carries fully-specified quicksort, the merge kernel, and round-trip codecs. Current state: **Tier 1 complete** (`Vec`/generics M7, traits/hash map M8) and **the Tier 2 spine landed** — UTF-8 codec + buffer validation (M9), JSON tokenizer (M10), option accessors (M11, ADR 0008), and the JSON parser against the full recursive grammar (M12 layer 1, 270 obligations). Next north star: **make the ordinary language and the proof layer scale** — type-precondition concepts for templates (verify generic code once, check the preconditions at instantiation), first-class class values (parameters, returns, class-valued fields), `#[label]`ed obligations — then the **bignum arithmetic pillar** (`Nat` cmp/add/sub, then schoolbook multiplication, the first benchmark where the mathematics itself is the test). The SVM semantic-oracle track (determinism, functional evaluator, differential testing against `interp.rs`) runs alongside on the core subset.
