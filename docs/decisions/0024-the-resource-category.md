...
## The sealed transformations

`split_off(&mut whole, n)` and `join(a, b)` are compiler-known, not
library functions and not prelude declarations. Each states a rule about
*who owns what*, and those rules are the compiler's; a program may not
define one (`split_off` and `join` are reserved names). Their contracts
are generated by vcgen over `SpanView.take`/`.drop`/`.cat`:

- `split_off` owes `0 ≤ n ∧ n ≤ whole.len` and rebinds the borrowed
  token to `whole.take n`, returning `whole.drop n`. **No product type is
  needed for the first resource slice**, which was the open question: one
  side goes back through the mutable borrow.
- `join` owes `a.alloc = b.alloc ∧ a.off + a.len = b.off` and returns
  `a.cat b`, consuming both.

**Adjacency and bounds are failed VCs, never checker errors.** The
checker has no idea where a span sits — it tracks tokens, not
geometry — and that division is the whole architecture in one rule.

Each argument of `join` moves as it is checked, not in a second pass.
Deferring the moves accepts `join(a, a)`, and the adjacency VC does *not*
catch it: an empty span is adjacent to itself, so a zero-length token
would be duplicated out of nothing. The corpus found this within minutes
of the operation existing.
...
- **Raw operations.** `load8`, `store8`, `take8`, `copy_nonoverlapping`,
  ...
  then a `RawSpan` can be moved, borrowed, carved, rejoined, and
  described, and that is all — which is precisely enough to shake out the
  ownership rules.
