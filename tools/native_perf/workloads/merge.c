...
static void merge(const int32_t *left, const int32_t *right, int32_t *output) {
  size_t left_index = 0;
...
int main(void) {
  static const int32_t left_input[LEFT_COUNT] = {1, 3, 5, 7};
  static const int32_t right_input[RIGHT_COUNT] = {2, 3, 6};
  static const int32_t expected[OUTPUT_COUNT] = {1, 2, 3, 3, 5, 6, 7};

  for (size_t iteration = 0; iteration < WORK_UNITS; ++iteration) {
    int32_t *left = __sable_rt_array_alloc_v1(sizeof(left_input));
    int32_t *right = __sable_rt_array_alloc_v1(sizeof(right_input));
    int32_t *output = __sable_rt_array_alloc_v1(sizeof(expected));
