...
- **Concepts** (M13, ADR 0009): type preconditions on templates — `/// requires T.max ≥ 100` — with **generic functions verified once against an abstract type model** and instantiations owing only the (automatic) precondition facts. C++ concepts check syntax; Sable concepts check lemmas. Plus `#[label(name)]`: stable semantic names for obligations and their hypotheses.
