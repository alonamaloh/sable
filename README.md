...
N2's real imported `Nat` addition closure is closed.
N3's real imported `Nat` subtraction and schoolbook-multiplication closures
are closed.
N4's real imported `Nat` division, remainder, and gcd closures are closed.
N5's exact nested signed-`Integer` lowering is closed.
...
only the formal-machine boundary. At that checkpoint native LLVM array lowering
remained a later independent stage; G1.6 below now supplies it.
...
G1.6 implements native lowering for that same owned-local Boolean-array
intersection. LLVM uses the internal descriptor
`%sable.array.bool = type { ptr, i64 }`, with an opaque data pointer followed
by the `u64` length. Elements are canonical zero/one `i8` bytes, never packed
`i1` storage. Nonempty storage crosses only the external versioned hooks
`__sable_rt_array_alloc_v1(i64 bytes)` and
`__sable_rt_array_free_v1(ptr)`; zero-length arrays use a null pointer and call
neither hook. The checked type retains the Boolean payload identity. The
repository's optional hosted shim, [`runtime/hosted/sable_rt_v1.c`](runtime/hosted/sable_rt_v1.c),
implements those hooks with the platform C allocator after rejecting a `u64`
byte count that does not fit `size_t`; generated IR never names `malloc` or
`free`.

Allocation evaluates length and then its Boolean initializer before checking
the 50,000,000-element profile cap or calling the hook. A contextual literal
evaluates every element left-to-right before allocating and writing canonical
bytes in order. Store order is index, value, unsigned bounds guard, then a
non-`inbounds` address calculation and write; a read likewise guards before
the address calculation and load. Cap exhaustion or a null hook result reaches
trap kind 9 with `(type_info, lhs, rhs) = (0, len, 0)`. An out-of-bounds read or
write reaches kind 10 with `(0, index, len)`. Both still pass through the
versioned observer and mandatory `llvm.trap`.

Function bodies, each `if` arm, and each `while` iteration are cleanup scopes.
Owned arrays are freed in reverse declaration order on normal exit and before a
loop backedge. A return evaluates its expression first, then unwinds active
scopes inner-to-outer. `unsafe { ... }` remains an open marker whose locals
belong to the enclosing scope, while traps terminate without cleanup, matching
the interpreter. Parameters, returns, fields, borrows, exposure, whole-array
rebinding or movement, calls, extern/public positions, generic or option
containment, discarded temporaries, and integer arrays all remain closed at
this backend boundary.

The lifetime work also closed a pre-existing owned-integer-array alias hole.
The checker path that moves a local into an array field now rejects a source
already marked moved; the interpreter correspondingly takes named owned-array
sources during transfer and removes owned-array locals and parameters when they
die. Thus moving one integer array into two fields cannot give two logically
independent sequence values the same backing storage. The new must-fail subject
brings the corpus inventory to 395 files: 83 verifies, 245 must-fail, 48 tests,
and 19 test-fails.
...
passed 7/7, including the strong-hook array fixture at Clang `-O0` and `-O2`;
the exact interpreter/native differential passed 1/1 over six subjects at both
optimization levels; and SVM differential remained 86/86. Randomized
...
That checkpoint intentionally granted no source-level affine-option behavior.
...
G2.1 implements only explicit mutable local `option<[bool]>` declarations.
Each declaration must be initialized by `none` or directly by
`some(alloc_array<bool>(len, init))`; wrapping an existing owned array and
Boolean-array literals remain closed. `.is_some` observes the tag without
consuming or cloning the payload, while program `.value` remains forbidden.
`.take` is an atomic named-place operation accepted only as the direct
initializer of an explicit owned `[bool]` local: it checks presence, transfers
the payload, and leaves the mutable source initialized as `none`. Parameters,
returns, calls, fields, traits, generics, borrows, exposure, inferred bindings,
whole-option assignment, nesting, non-Boolean payloads, and discarded affine
temporaries remain rejected.

VC generation represents the local as `Option (Sable.Seq Bool)`, proves
someness against the pre-take snapshot, and updates the symbolic source to
typed `none`; loop effect collection treats take as a source mutation. The
interpreter uses a runtime variant separate from copy options and takes from
the named slot atomically. Scope exit recursively destroys a still-present
payload exactly once, while the dynamic proof monitor observes immutable
snapshots rather than executable owners. Affine payload clauses use `match`;
affine `.value` is unmonitorable because `Sable.Seq` intentionally has no
global `Inhabited` instance, so the monitor rejects it rather than accepting a
clause Lean cannot elaborate.
...
1/1 spanning six subjects at Clang `-O0` and `-O2`; and SVM differential
remained 86/86. Randomized free-list allocator, grind-budget, LSP,
documentation, rustfmt, diff-check, and static-audit gates were green. G2.1 is
closed.
ADR 0061 records the atomic transition in
[`docs/decisions/0061-affine-option-take-is-an-atomic-place-transition.md`](docs/decisions/0061-affine-option-take-is-an-atomic-place-transition.md).

G2.2's closed formal-core slice uses statement-level
`Stmt.optTake dst src` over the existing recursive `Val.opt`. For distinct
names, a present payload is transferred in one transition which clears the
source to `none` and installs the payload in the destination; `none` traps with
`optionNone`, while a missing or wrong outer value is `undef`. Source and
destination aliasing is immediately `undef`. The destination need not be
absent: the SVM's flat environment reuses lexical-local names across loop
iterations, so a stale destination binding is overwritten. The transition
preserves the tagged Boolean-array payload even when its length is zero.

The formal core is deliberately generic over the recursively represented
payload, while Rust lowering is the exact supported-subset gate: only the
G2.1 `option<[bool]>` source and owned `[bool]` destination lower to
`optTake`. Parameters, returns, calls, fields, traits, generics, borrows,
exposure, whole-option movement, and every affine-option ABI remain fenced.
...
were green. G2.3 is closed.

N0 begins the native `Nat` ladder without admitting classes. It adds the exact
internal `%sable.array.u32 = type { ptr, i64 }` representation for fresh owned
local `[u32]` literals and `alloc_array<u32>` values, with checked `.len`,
index reads, and stores. Internal free functions may accept `&[u32]` and
`&mut [u32]`; call sites must carry the exact explicit named borrow with matching
mutability, and those parameters never own or free the caller's descriptor.
Owned-array parameters and returns, fields, classes, externs, entries, and every
other element type stay closed, so this is not an array ABI.

N0 reuses the versioned array hooks, 50,000,000-element cap, zero-length hook
bypass, reverse lexical cleanup, and trap kinds 9/10. Allocation passes
`len * 4` bytes but reports logical element counts in traps. The v1 allocation
hook promises only byte storage, so every native `u32` element load and store is
explicitly `align 1`; N0 does not retroactively strengthen the runtime ABI.
The interpreter and existing verification model already define this source
slice, so no new VC or Lean rule is introduced. A verified interpreter/native
fixture and a strong-hook Clang matrix cover literals, nonzero fill, shared and
mutable borrows, zero, reverse branch and loop cleanup, early return, OOB, OOM,
and profile-cap precedence at `-O0` and `-O2`.

N0 closed under the exact one-worker command
`CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 SABLE_TEST_JOBS=1 SABLE_LEAN_JOBS=1
SABLE_REQUIRE_CLANG=1 cargo test -j1 -- --test-threads=1 --nocapture`.
`cargo check` was green; focused LLVM units passed 31/31; and Rust library
tests passed 215/215. The recursive corpus remained green across all 416 files
(84 verifies, 263 must-fail, 49 tests, 20 test-fails) in 213.51s. LLVM CLI
passed 9/9; the exact interpreter/native differential passed 1/1 over eight
subjects at Clang `-O0` and `-O2`; and SVM differential remained 92/92.
...
same N0–N2 machinery. Subtraction uses one scratch `[u32]` array and checked
borrow arithmetic; multiplication uses one scratch array, nested scalar loops,
and checked product/carry arithmetic. Both trim the scratch prefix, construct
the result with `Nat::from_prefix`, return through the hidden destination, and
neutralize the named result local before ordinary reverse lexical cleanup. No
new runtime hook, class representation, ownership rule, or ABI is introduced.

The dedicated N3 fixture verifies 51/51 obligations across 19 functions and
returns 42 when compiled directly with Clang at both `-O0` and `-O2`. It covers
subtraction to zero, a borrow chain across two zero limbs, multiplication by
zero, a maximum limb squared, and cross-limb carry.

N4 carries the real imported `div`, `rem`, and `gcd` closures through mutable
locals of the same exact fixed `Nat` shape. Each reassignment target has one
scratch slot hoisted to the function entry. The complete right-hand side is
evaluated into that scratch first, so expressions such as `dd = dd - vn` and
`q = shift_in(&q, d)` may safely borrow the old destination. Only then is the
old owner dropped; the replacement aggregate moves into the target and the
scratch is zeroed. This preserves one live owner without allocating stack
storage on every iteration.

The existing cleanup registry needs no parallel loop mechanism. Owners declared
inside a loop body are destroyed in reverse lexical order before its backedge,
outer mutable owners remain function-scoped, and zeroed named-move sources are
null-safe at their registered cleanup; the reassignment scratch is deliberately
unregistered and empty after each transfer. Move-state validation still requires
every reaching branch and loop backedge to restore an unambiguous owner-liveness
shape; reassignment may revive a moved mutable target without permitting a read
or borrow while it is moved.

The dedicated N4 fixture verifies 109/109 obligations across 21 selected
functions with six small hand discharges. It covers division by one, exact and
inexact quotient/remainder pairs, the multi-limb quotient-estimate correction,
and basic, zero-input, and coprime gcd cases. Its emitted program returns 42
when compiled directly with Clang at both `-O0` and `-O2`. N4 adds no runtime
hook, class representation, aggregate transport ABI, or proof rule.

N5 implements the exact nested signed shape
`Integer { Nat mag; u64 neg; }` as an internal aggregate containing the
already-pinned `Nat` aggregate followed by an `i64` scalar. Initializer
validation is per field rather than one class-wide bit: `mag` and `neg` must
each be initialized exactly once on every reaching path, and scalar field
stores cannot accidentally stand in for initialization of the owned field.
Class-field projection now covers scalar reads and the exact borrows used by
the real imported implementation: `&x.mag` borrows the nested `Nat`, while
`&a.limbs` borrows its array descriptor without transferring either owner.

The internal take convention admits exact owned `Nat` parameters. A caller
passes the aggregate by value and neutralizes a named source; the callee stores
it in an owning slot with ordinary lexical cleanup and may consume that slot
into `Integer.mag`. Class-returning argument expressions use a unique
entry-hoisted, unregistered scratch destination. The completed aggregate is
loaded by value and the scratch is zeroed immediately before the call,
transferring ownership to the callee without a caller-side post-call drop.
Mutable `&mut Integer` remains a non-owning pointer, and method lowering is
opened only for the exact private
`Integer::flip_sign(&mut self)` method used by `negate_in_place`. Its scalar
`neg` update therefore exercises mutation without granting general method or
class-reference support.

Destruction recurses through supported class fields in reverse declaration
order. Dropping an `Integer` visits scalar `neg` (a no-op) and then drops
`mag`, whose `limbs` descriptor performs the existing null-checked free.
Moved named owners and callee parameter slots are zeroed, so their later
registered cleanups cannot free the nested allocation twice; the unregistered
argument scratch is likewise empty before control enters the callee.

`corpus/verifies/integer_native.sable` verifies 237/237 obligations across 39
selected functions and covers construction, unary operations, in-place sign
mutation, addition, subtraction, multiplication, and Euclidean division and
remainder under all sign combinations. Its emitted program returns 42 when
compiled directly with Clang at both `-O0` and `-O2`. A dedicated strong-hook
lifetime test is green at both optimization levels: the program exits 42 with
`live = 0`, while the hook aborts on leaks, unknown frees, or double frees. The
exact `VerifiedProgram` differential is green 1/1 over 13 subjects at both
optimization levels, including `Integer` exit 42.

N5 closed under the gentle serial command
`SABLE_REQUIRE_CLANG=1 cargo test -j1 -- --test-threads=1`. It passed 223/223
library tests; the corpus gate 1/1 in 93.64s; randomized allocator 1/1;
grind-budget 1/1; LLVM CLI 10/10; the differential gate 1/1 in 31.35s; LSP
1/1; SVM differential 1/1; and documentation tests. `cargo check -j1`,
rustfmt, the focused verifier fixture, and the dedicated lifetime test were
green as well. N5 is closed.

This is still an internal exact-declaration convention, not a class ABI.
Owned `Integer` parameters, arbitrary owned class parameters, methods other
than the exact `flip_sign` closure, mutable borrows of other classes, discarded
class results, moves from fields, additional or generic class shapes, nonempty
destructors, array whole-value transport, and public, extern, or cross-module
class ABIs remain fenced. Generic owner slots and `Vec` remain a separate later
design.
