...
- **First-class class values** (M14, ADR 0010): shared class borrows, class returns, and field reads — `fn union_of(&Range a, &Range b) -> Range` verifies with contracts written directly over class structures (`result.lo ≤ a.lo`), borrowed arguments carrying their invariants in and returned values carrying them out as checked obligations. The bignum prerequisite.
